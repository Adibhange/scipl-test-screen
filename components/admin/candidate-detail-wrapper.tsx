...
export function CandidateDetailWrapper({
	...
	async function saveRound(round: "face_to_face" | "assessment" | "director", status: "pass" | "fail", remarks: string) {
		...
		if (response.ok) {
				...
				setTimeout(async () => {
					try {
						const fresh = await fetch(`/api/admin/candidates/${result.candidate.id}`);
						if (fresh.ok) {
							const freshData = await fresh.json();
							if (freshData?.candidate) setResult(prev => ({ ...prev, candidate: { ...prev.candidate, hiringStatus: freshData.candidate.hiringStatus } }));
						}
					} catch {}
				}, 800);
			} else {
	...
	async function saveRoundInterviewer(
		...
		try {
			const response = await fetch("/api/admin/assignment", {
				method: "PATCH",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ resultId: result.id, interviewerEmail, interviewerName }),
			});
			if (response.ok) {
				const nextResult = await response.json();
				setResult(nextResult);
