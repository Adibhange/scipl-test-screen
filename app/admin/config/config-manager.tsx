...
export function ConfigManager() {
  ...
  async function handleCreateVacancy(e: React.FormEvent) {
    ...
    try {
      ...
      setVacancies(prev => [...prev, { ...data, applicantCount: 0 }])
      setVacancyForm({
        role: "",
        customRole: "",
        experience: "",
  ...
  async function toggleVacancyActive(vacancy: JobVacancy) {
    try {
      ...
      if (!res.ok) throw new Error("Failed to update vacancy status")

      setVacancies(prev => prev.map(v => v.id === vacancy.id ? { ...v, is_active: !v.is_active } : v))
    } catch (err: unknown) {
  ...
  async function handleSaveOpenings(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.error ?? "Failed to update openings")
      }

      setVacancies(prev => prev.map(v => v.id === id ? { ...v, openings: num } : v))
  ...
  async function handleDeleteVacancy(id: string) {
    ...
    try {
      const res = await fetch(`/api/admin/config?id=${id}&isVacancy=true`, {
        method: "DELETE"
      })
  ...
  async function handleAddMaster(type: ConfigItem["type"], fields: Record<string, string>) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? "Failed to add configuration")
  ...
  async function handleQuickAddTestVenue() {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? "Failed to add test venue")
  ...
  async function toggleMasterActive(item: ConfigItem) {
    try {
      ...
      setConfigs(prev => prev.map(c => c.id === item.id ? { ...c, is_active: !c.is_active } : c))
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Failed to update status"
      alert(msg)
    }
  ...
  async function handleDeleteMaster(id: string, type: ConfigItem["type"]) {
    ...
    try {
      const res = await fetch(`/api/admin/config?id=${id}&type=${type}`, {
        method: "DELETE"
      })
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs space-y-4">
              ...
              <form onSubmit={handleCreateVacancy} className="space-y-4 text-xs font-bold text-slate-650">
                ...
                <Button
                  type="submit"
                  className="group w-full h-11 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold shadow-md transition-all duration-300 hover:shadow-lg hover:scale-[1.02] hover:from-indigo-700 hover:to-violet-700 active:scale-[0.98]"
                >
